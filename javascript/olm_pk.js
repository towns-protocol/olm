    ...
    return function () {
        ...
        if (result === OLM_ERROR) {
            var message = UTF8ToString(
                _olm_pk_encryption_last_error(arguments[0])
            );
...
PkEncryption.prototype["encrypt"] = restore_stack(function (plaintext) {
    ...
    try {
        ...
        ciphertext_buffer = malloc(
            ciphertext_length + NULL_BYTE_PADDING_LENGTH
        );
        ...
        var ephemeral_buffer = stack(
            ephemeral_length + NULL_BYTE_PADDING_LENGTH
        );
...
function pk_decryption_method(wrapped) {
    return function () {
        ...
        if (result === OLM_ERROR) {
            var message = UTF8ToString(
                _olm_pk_decryption_last_error(arguments[0])
            );
            throw new Error("OLM." + message);
        }
        return result;
...
PkDecryption.prototype["decrypt"] = restore_stack(function (
    ...
    try {
        ...
        plaintext_max_length = pk_decryption_method(
            _olm_pk_max_plaintext_length
        )(this.ptr, ciphertext_length);
...
function pk_signing_method(wrapped) {
    return function () {
        ...
        if (result === OLM_ERROR) {
            var message = UTF8ToString(
                _olm_pk_signing_last_error(arguments[0])
            );
            throw new Error("OLM." + message);
        }
        return result;
